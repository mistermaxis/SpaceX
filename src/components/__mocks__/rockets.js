        ...
        rockets.map((item) => (
          <div key={item.id} className="container-fluid d-flex my-4 mx-0 mainDiv">
            ...
            <div>
              ...
              <div>
                <button bg="info">{handleReserved(item.reserved).badge}</button>
                <p>{item.description}</p>
              </div>
              <button className="btn2" type="button" variant={handleReserved(item.reserved).class} onClick={() => { dispatch(!item.reserved ? bookRocket(item.id) : cancelBooking(item.id)); }}>{handleMessage(item.reserved)}</button>
            </div>
          </div>
        ))
      }
